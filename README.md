...
Add `https://github.com/cyijun/china-financial-services` as a plugin source. Kimi will read `.kimi-plugin/plugin.json` at the repo root and install the `china-financial-services` meta-plugin, which exposes an entry skill describing all included sub-plugins.
