...
Add `https://github.com/cyijun/china-financial-services` in Kimi Code plugin marketplace settings. The marketplace downloads the latest release zips automatically. Then install the plugins you want:
...
#### Releasing Kimi plugins

When you create a GitHub Release, `.github/workflows/release-kimi-plugins.yml` automatically packages each plugin into a zip and attaches it to the release. The root `marketplace.json` always points to the latest release assets.
