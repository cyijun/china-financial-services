...
REQUIRED_RISK_FREE_EVIDENCE = {"observation_date", "term_years", "currency", "provider", "interface", "curve_type", "source_value", "source_unit"}
...
def date_value(value: Any, field: str) -> date:
...
def validate_risk_free_rate_evidence(capital: Mapping[str, Any], valuation_date: Any) -> Dict[str, Any]:
    evidence = capital.get("risk_free_rate_evidence")
    if not isinstance(evidence, Mapping):
        raise ValueError("capital.risk_free_rate_evidence must be a mapping")
    missing = REQUIRED_RISK_FREE_EVIDENCE - set(evidence)
    if missing:
        raise ValueError(f"missing risk-free evidence fields: {sorted(missing)}")
    observed = date_value(evidence["observation_date"], "risk_free_rate_evidence.observation_date")
    valued = date_value(valuation_date, "valuation_date")
    if observed > valued:
        raise ValueError("risk-free observation_date cannot be after valuation_date")
    term = number(evidence["term_years"], "risk_free_rate_evidence.term_years")
    if term <= 0:
        raise ValueError("risk-free term_years must be positive")
    currency = str(evidence["currency"]).strip().upper()
    if currency != "CNY":
        raise ValueError("China A-share DCF risk-free evidence must use CNY")
    provider = str(evidence["provider"]).strip()
    interface = str(evidence["interface"]).strip()
    if not provider or not interface:
        raise ValueError("risk-free evidence requires non-empty provider and interface")
    curve_type = str(evidence["curve_type"]).strip().lower()
    if curve_type not in {"0", "1", "maturity", "spot"}:
        raise ValueError("risk-free curve_type must be maturity/0 or spot/1")
    source_value = number(evidence["source_value"], "risk_free_rate_evidence.source_value")
    source_unit = str(evidence["source_unit"]).strip().lower()
    if source_unit == "percent":
        converted = source_value / 100.0
    elif source_unit == "decimal":
        converted = source_value
    else:
        raise ValueError("risk-free source_unit must be percent or decimal")
    configured = number(capital["risk_free_rate"], "risk_free_rate")
    if not math.isclose(configured, converted, rel_tol=1e-12, abs_tol=1e-12):
        raise ValueError("risk_free_rate does not match the unit-converted source evidence")
    return {
        **dict(evidence),
        "observation_date": observed.isoformat(),
        "valuation_date": valued.isoformat(),
        "term_years": term,
        "currency": currency,
        "curve_type": "maturity" if curve_type in {"0", "maturity"} else "spot",
        "converted_rate_decimal": converted,
    }
...
def calculate_wacc(capital: Mapping[str, Any]) -> Dict[str, float]:
    ...
    if not -1 < rf < 1:
        raise ValueError("risk_free_rate must be a decimal rate between -1 and 1, not a percentage value")
...
def run(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    risk_free_evidence = validate_risk_free_rate_evidence(config["capital"], config["valuation_date"])
    ...
    return {
        ...
        "risk_free_rate_evidence": risk_free_evidence,
        "wacc": wacc,
        "forecast": forecast,
        "valuation": valuation,
        "sensitivity": table,
        "checks": {
            ...
            "risk_free_rate_evidence_matches_input": math.isclose(number(config["capital"]["risk_free_rate"], "risk_free_rate"), risk_free_evidence["converted_rate_decimal"], rel_tol=1e-12, abs_tol=1e-12),
