...
description: 为中国A股、公募基金和人民币利率研究选择并调用Tushare Pro或AKShare数据源，输出带接口、参数、抓取时间、响应哈希、可得时点和降级记录的结构化数据。用于股票/ETF行情、基金净值、财务、估值、行业、国债收益率曲线及回测数据准备；严格PIT任务不得静默使用不具备历史可得时点的备用数据。
---
