...
- 曲线观测按`trade_date`过滤；估值和回测不得使用晚于`as_of`的记录。
- 数据层保留收益率百分数和期限年数；转换为模型小数必须保留源值、源单位和转换结果。
- AKShare历史标准期限到期曲线不能证明历史即期曲线；近期精细接口覆盖之外必须失败关闭。
- 对需要证明当时网页内容或防止后续修订的研究，保存原始响应哈希或归档快照。
